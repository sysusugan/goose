...
goose 主要由三个部分组成：**界面（interface）**、**agent** 和已连接的[扩展](/docs/getting-started/using-extensions)。
...
goose 内置了一批[内建扩展](/docs/getting-started/using-extensions#built-in-extensions)，覆盖开发、网页抓取、自动化、记忆等场景；同时也支持接入外部扩展，或者通过 [custom extensions](/docs/tutorials/custom-extensions) 自定义 MCP server。
