...
会话 ID 采用 `YYYYMMDD_<COUNT>` 格式，例如 `20250310_2`。goose CLI 会在每个会话开始时输出当前会话 ID。要查看所有会话 ID，可以使用 [`goose session list` 命令](/docs/guides/goose-cli-commands#session-list-options)。

另外也可以参考[会话管理](/docs/guides/sessions/session-management)，了解如何查找和管理会话。

## 系统日志

goose 会为不同组件保存日志。CLI 和 server 日志会按日期分目录存储，并在两周后自动清理，防止占用过多磁盘空间。

如果启用了[提示注入检测](/docs/guides/security/prompt-injection-detection)，CLI 和 server 日志还会包含：
...
Desktop 遵循平台惯例保存自身运行日志和状态数据，但真正的会话内容仍使用标准的 [会话记录](#会话记录)。这意味着无论你通过哪种界面使用 goose，你的会话历史都在同一套存储里。
