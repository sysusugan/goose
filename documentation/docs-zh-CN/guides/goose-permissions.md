...
| **Manual Approval** | goose 在使用任何工具或扩展前都会**逐项请求确认**（支持细粒度[工具权限](/docs/guides/managing-tools/tool-permissions)） | 希望**逐项审查并批准**每次变更和工具调用的用户 |
| **Smart Approval** | goose 基于风险判断，**自动放行低风险操作**，并对其他操作请求审批（支持细粒度[工具权限](/docs/guides/managing-tools/tool-permissions)） | 希望在自治和可控之间取得**平衡**的用户 |
